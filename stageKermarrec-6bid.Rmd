...
### representation du packing sur les structures 3D.
...
```{r}
resSel <- gsub("_A", "", colnames(matrice)[grep("_A", colnames(matrice))])
resSel2 <- paste(resSel, collapse="+")
paste("select respackA, resid ", resSel2,  " and chain A", sep="")
```


2. tous les résidus conservés chez PR1 et PR2
même procédure 
...
3. ceux qui sont spécifiques à PR2
ceux qui sont beaucoup vus chez PR2 et pas chez PR1

Bien lister les résidus qui ont été colorés



4. ceux qui sont spécifiques à PR1 (structure de PR1 complexé au DRV : PDB code : 2ien)
ceux qui sont beaucoup vus chez PR1 et pas chez PR2


![legende](figures/structure_2ien.png){width=65%}



Faire pareil pour les résidus spécifique de PR1 : 
