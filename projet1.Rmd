...
On refait la classification mais on colore en bleu pour les PR1
```{r}
mat2 <- matrice[-27:-29,]

mat2.PR1 <- mat2[names(which(type=="PR1")),]
mat2.PR1[which(mat2.PR1==1)]=2


mat2 = rbind(mat2.PR1, mat2[names(which(type=="PR2")),])

svg("figures/heatmap_resPack.svg", width=18)
pheatmap(mat2, cluster_rows = TRUE, cluster_cols = FALSE, br=-1:2, col=c("white", "red", "blue"),
         clustering_distance_rows = "binary")
dev.off()
```
...
On sépare la chaine A et chaine B
```{r}
svg("figures/heatmap_resPack_chainA.svg", width=18)
supp.apo = c(1, 7, 13, 18)
mat2.ssapo = mat2[-supp.apo,]
resA <- grep("_A", colnames(mat2))
pheatmap(mat2.ssapo[,resA], cluster_rows = TRUE, cluster_cols = FALSE, br=-1:2, col=c("white", "red", "blue"),
         clustering_distance_rows = "binary")
dev.off()


svg("figures/heatmap_resPack_chainB.svg", width=18)
resB <- grep("_B", colnames(mat2))
pheatmap(mat2.ssapo[-c(1,7,13),resB], cluster_rows = TRUE, cluster_cols = FALSE, br=-1:2, col=c("white", "red", "blue"),
         clustering_distance_rows = "binary")
dev.off()
...
On classe les protéines suivant leur résidus impliqués dans le packing.
Pour cela, on réalise une classification hiérarchique utilisant la distance "binary" et la méthode d'aggrégation Ward.
Les distances sont calculées en utilisant la matrice `matrice` qui contient : 

* en lignes les protéines
* en colonnes les résidus
* dans chaque case 0 ou 1 : avec 1 si le résidu est impliqué dans le packing de la protéine étudiée.

```{r}
hc.res1 <- hclust(dist(matrice[-27:-29,], method = "binary"), method="ward.D2")
plot(hc.res1, sub="", xlab="")
```

On voit que : 
* 1ère séparation : les 3 protéines à se séparer des autres sont les trois formes apo de PR1 (car pas de chaine B).
* 2ème séparation : cinq PR1 qui se séparent des autres PR1/PR2
* 3ème séparation : 7 PR1 qui se séparent des 11 PR2

*To do : Regarder si les protéines de ces 5 groupes appartiennent au même espace cristallin.*
...
![Une protéase du VIH 2 avec uniquement les résidus PR2 impliqués dans la structure](3ebz etude comparative.png){width=65%}
*To do: donner le code couleur.* 


```{r}
resSelA = resSelB = resSel_B2_PR1 = resSel_A2_PR1 = resSel_A_PR1 = resSel_B_PR1 = NULL
      ...
      if (matrice[i,j] == 1){
        for (k in 1:length(resA)) {
          ...
          if(j == resA[k]){
            ...
            resSelA = c(gsub("_A", "", j))
            resSel_A_PR1 = paste(resSelA, collapse="+")
            resSel_A2_PR1 = unique(c(resSel_A2_PR1,resSel_A_PR1))
          }
        }
        for (g in 1:length(resB)) {
          ...
          if(j == resB[g]){
            ...
            resSelB = c(gsub("_B", "", j))
            resSel_B_PR1 = paste(resSelB, collapse="+")
            resSel_B2_PR1 = unique(c(resSel_B2_PR1,resSel_B_PR1))
          }
        }
    }
  }
}
resSel_B2_PR1 = paste(resSel_B2_PR1, collapse="+")
resSel_A2_PR1 = paste(resSel_A2_PR1, collapse="+")

paste("select respackA, resid ", resSel_A2_PR1,  " and chain A", sep="")
paste("select respackB, resid ", resSel_B2_PR1,  " and chain B", sep="")
```
...
```
select respackB, resid  and chain B
...
![Une protéase du VIH 2 avec tous les résidus impliqués dans la structure](pymol3ebz.png){width=65%}

Légende: Les résidus en Vert ne sont pas impliqués dans le packing cristallin
         Les résidus en Rouge sont impliqués dans le packing cristallin de la chaine A
         Les résidus en Bleu sont impliqués dans le packing cristallin de la chaine A


![Une protéase du VIH 2 avec tous les résidus impliqués dans la structure](3ebz etude comparative.png){width=65%}

Légende: Les résidus en Vert ne sont pas impliqués dans le packing cristallin
         Les résidus en Rouge sont impliqués dans le packing cristallin de la chaine A chez les prtéases de type 2
         Les résidus en Bleu sont impliqués dans le packing cristallin de la chaine B chez les prtéases de type 2
         Les résidus en Jaune sont impliqués dans le packing cristallin de la chaine A chez toutes les protéases (donc ces résidus représentent les résidus qui sont chez PR1 mais pas chez PR2)
         Les résidus en Magenta sont impliqués dans le packing cristallin de la chaine B chez toutes les protéases (donc ces résidus représentent les résidus qui sont chez PR1 mais pas chez PR2)
...
Pour chaque résidu, on regarde si le hasard peut expliquer la forte occurrence du résidu.
...
```{r}
resA.1 <-colnames(MatricePR1)[grep("_A", colnames(MatricePR1))]
resB.1 <- colnames(MatricePR1)[grep("_B", colnames(MatricePR1))]
...
# Détermination des résidus impliqués dans le packing cristallin chez PR2
...
```{r}
pourc.PR2 <- round(occ.PR2/ nrow(MatricePR2)*100,2)
...
## résidus impliqués dans le packing et conservés chez PR1 et PR2
```{r}
consvPR1.PR2 <- intersect(res.conserv.PR1, res.conserv.PR2)
length(consvPR1.PR2)
```
Il y a `r length(consvPR1.PR2)` résidus qui sont impliqués dans le packing chez la plupart des PR1 mais pas chez PR2 : `r consvPR1.PR2`.
...
##Calcul les différences de pourcentage chez PR1 et PR2


Ce sont ces résidus que l'on a conservé pour le papier RMSD
```{r}
diff.purc = pourc.PR1 - pourc.PR2
pos.ord.sel2 <- names(which(sort(abs(diff.purc))>60))
barplot(abs(diff.purc[pos.ord.sel2]),las=2)
```

Il y a `r length(pos.ord.sel2)` résidus qui sont impliqués dans le packing spécifiquement chez une PR
...
Détermine si les résidus sont mutés
```{r}
mut.diff11 <- rep("no", length = length(pos.ord.sel2))
names(mut.diff11) <- pos.ord.sel2
mut.diff11[intersect(res.muT, pos.ord.sel2)] = "yes"
```


```{r}
tmp.occ <- data.frame(pos.ord.sel2, 
                      occ.PR1[pos.ord.sel2], 
                      occ.PR2[pos.ord.sel2],
                      regionV[pos.ord.sel2], 
                      t(matrice3PR1[,pos.ord.sel2]),
                      t(matrice3PR2[,pos.ord.sel2]),
                      mut.diff11
                      )
colnames(tmp.occ) <- c("residues", "PR1.Occ", "PR2.Occ", "loc", 
                       "BB.PR1", "SC.PR1",
...
Pour le papier, il faut travailler sans les apo

* supprime les apo

```{r}
apo.ind.PR1 <- c(7, 1, 13)
MatricePR1.holo <- MatricePR1[-apo.ind.PR1,]

occ.PR1.apo <- apply(MatricePR1.holo,2,sum)
pourc.PR1.apo <- occ.PR1.apo/ nrow(MatricePR1.holo)


apo.ind.PR2 <- 3
MatricePR2.holo <- MatricePR2[-apo.ind.PR2,]

occ.PR2.apo <- apply(MatricePR2.holo,2,sum)
pourc.PR2.apo <- occ.PR2.apo/ nrow(MatricePR2.holo)
```


```{r}
diff.purc = pourc.PR1.apo - pourc.PR2.apo
hist(diff.purc)
pos.ord.sel2 <- names(which(sort(abs(diff.purc))>0.6))
barplot(abs(diff.purc[pos.ord.sel2]),las=2)
```


```{r}
pourc.PR1.apo[pos.ord.sel2]
```
...
### Identification des résidus de packing spécifiques chez PR1 et PR2 par une approche de Random forest

```{r}
#regarde s'il y a des variables avec variance nulle
v.var <- apply(matrice[-27:-29,],2,var)

var.supp <- which(v.var ==0)

if(length(var.supp)!=0){
  mat.tmp <- matrice[-27:-29,-var.supp]
}else{
  mat.tmp <- matrice[-27:-29,-var.supp]
}

dim(mat.tmp)
mat.app <- data.frame(mat.tmp, as.factor(type[rownames(matrice)]))
colnames(mat.app) <- c(paste("p",colnames(mat.tmp),sep="."), "PR.type")

rf.fit <- randomForest(PR.type~., data = mat.app)
plot(rf.fit)


v.err <-  NULL
v.mtry <- c(3,6, 12, 24, 48, 84, 150)
for(i in v.mtry){
    set.seed(123)
    rf.fit <- rf.fit <- randomForest(PR.type~., data = mat.app, ntree = 250, mtry=i)
    v.err <- c(v.err, rf.fit$err.rate[250,1])
}
...
rf.fit <- randomForest(PR.type~., data = mat.app, ntree = 150, ntry=90, importance=T)
print(rf.fit)


imp<- importance(rf.fit, scale=T)[,"MeanDecreaseAccuracy"]
varImpPlot(rf.fit, type=1)

(var.sel <- names(which(imp>2)))
```

```{r}
imp.500RF <- rep(0,length = (ncol(mat.app)-1))
names(imp.500RF) <- colnames(mat.app)[-ncol(mat.app)]

matImp <- matrix(NA, nrow=500, ncol = length(imp.500RF))
colnames(matImp) = names(imp.500RF)

for (n in 1:500){
  rf.fit.tmp <- randomForest(PR.type~., data = mat.app, ntree = 150, ntry=90, importance=T)
  imp<- importance(rf.fit.tmp, scale=T)[,"MeanDecreaseAccuracy"]
  matImp[n,names(imp)] = imp
  imp.500RF[names(which(imp>0))] = imp.500RF[names(which(imp>0))]+1
}



barplot(sort(imp.500RF), las=2)

nn = names(sort(imp.500RF))

boxplot(matImp[,nn])

mean.VImp = sort(apply(matImp,2,mean))
hist(mean.VImp)


val.selImp1 <- names(which(mean.VImp>2))
length(val.selImp1)


boxplot(matImp[,val.selImp1],las=2)


barplot(imp.500RF[val.selImp1], las=2)
summary(imp.500RF[val.selImp1]/500)
```
...
count.sup <- rep(0, length = ncol(mat.tmp))
names(count.sup) = paste("p",colnames(mat.tmp),sep=".")

niter = 10000
for(i in 1:niter){
    mat.app.alea <- data.frame(mat.tmp, as.factor(sample(type)))
    colnames(mat.app.alea) <- c(paste("p",colnames(mat.tmp),sep="."), "PR.type")
    rf.fit.alea <- randomForest(PR.type~., data = mat.app.alea, ntree = 150, ntry=90, importance=T)
    imp.alea <- importance(rf.fit.alea, scale=T)[,"MeanDecreaseAccuracy"]
    diff.imp <- imp.alea - imp
    count.sup[which(diff.imp>0)] = count.sup[which(diff.imp>0)]+1
}

pval <- count.sup / niter
length(which(pval<0.05/length(count.sup)))

(var.sel.pval <- which(pval<0.05/length(count.sup)))

```


caractérisation de ces positions

Détermine si les résidus sont mutés
```{r}
var.sel <- gsub("p.","",var.sel)
mut.diff <- rep("no", length = length(var.sel))
names(mut.diff) <- var.sel
mut.diff[intersect(res.muT, var.sel)] = "yes"
```


```{r}
tmp.occ <- data.frame(var.sel, 
                      occ.PR1[var.sel], 
                      occ.PR2[var.sel],
                      regionV[var.sel], 
                      t(matrice3PR1[,var.sel]),
                      t(matrice3PR2[,var.sel]),
                      mut.diff
                      )
colnames(tmp.occ) <- c("residues", "PR1.Occ", "PR2.Occ", "loc", 
                       "BB.PR1", "SC.PR1",
...
* descripteurs géométriques : Volume, surface, forme
```{r}
...
* descripteurs physicochimiques : hydrophobicité, charge, polarité, ...
```{r}
...
On supprime `r length(var.sup)` : `r colnames(mat.DescLig)[var.sup]`
...
1sdt | PR1 | P 21 21 2      1.3
2hb3 | PR1 | P 21 21 2      1.35


1hhp | PR1 | P 41 21 2      2.7
2hb4 | PR1 | P 41 21 2      2.15
3phv | PR1 | P 41 21 2      2.7


3ebz | PR2 | C 1 2 1        1.2
3ec0 | PR2 | C 1 2 1        1.18
3ecg | PR2 | C 1 2 1        1.18
3s45 | PR2 | C 1 2 1        1.51


3nu3 | PR1 | P 21 21 2      1.02
2z4o | PR1 | P 21 21 2      1.6
2ien | PR1 | P 21 21 2      1.3
...
1ivp | PR2 | P 21 21 21     2.5
1hih | PR1 | P 21 21 21     2.2
1hii | PR2 | P 21 21 21     2.3
1hiv | PR1 | P 21 21 21     2.0
3ekv | PR1 | P 21 21 21     1.75


2hpe | PR2 | P 1 21 1       2.0
1hsh | PR2 | P 1 21 1       1.9
1hsi | PR2 | P 1 21 1       2.5
1hsh | PR2 | P 1 21 1       1.9

Molécules étudiés:
2hpe, 2hpf, 2nph, 2mip, 1hiv, 1ivp, 1sdt, 1hsh, 2z4o, 3ecg, 2hb3, 3ec0, 4ll3, 4hla, 2ien, 3ebz, 1hii, 1hih, 3s45, 3ekv, 3nu3


groupe 1 (selon espace cristallin): 2hpe, 2hpf

groupe 2 (selon espace cristallin): 2nph-pA, 2nph-pB

groupe 3 (selon espace cristallin): 2mip_AB_pB, 2mip_CD_pB, 2mip_CD_pA, 2mip_AB_pA (DONC 2MIP)

groupe 4 (selon espace cristallin): 1hiv, 1ivp, 1sdt, 1hsh_AB, 1hsh_CD

groupe 5 (selon espace cristallin): 2z4o, 3ecg, 2hb3, 3ec0, 4ll3, 4hla, 2ien, 3ebz, 1hii, 1hih, 3s45, 3ekv, 3nu3

Les molécules correspondant au numéro: 1hhp,2hb4,3phv,2z4o,1hpv,1hsi ne seront pas étudiés.
...
Groupe 1 rangé selon la classification hiérarchique:

2hpe | PR2 | P 1 21 1       2.0
2hpf | PR2 | P 65           3.0


Groupe 2 rangé selon la classification hiérarchique:

2nph | PR1 | P 61           1.65


Groupe 3 rangé selon la classification hiérarchique:

2mip | PR2 | P 43           2.2


Groupe 4 rangé selon la classification hiérarchique:
...
Groupe 5 rangé selon la classification hiérarchique:
