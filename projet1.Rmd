...
for (i in NbProt) {
  ...
  for (j in 1:length(listAtomSyn)) {
    
    for (k in 1:length(ResidueTotaux)) {
      if (listAtomSyn[j] == ResidueTotaux[k]){
        names(listAtomSyn[j]) = names(ResidueTotaux[k])
        matrice3PR[i,names(listAtomSyn[j])] = matrice3PR[i,names(listAtomSyn[j])] + 1
      }
    }
  }
}
